...
def make_resume(name, email, path='output.pdf'):
    year = date.today().year

    font_face = 'Arial'  # Use a consistent font for a cleaner look

    pdf = FPDF()
    pdf.add_page()

    # Header
    pdf.set_font(font_face, 'B', 24)
    pdf.set_text_color(0, 51, 102)  # Dark blue color for the header
    pdf.cell(0, 10, name, ln=True, align='C')
    pdf.set_font(font_face, '', 12)
    pdf.set_text_color(0, 0, 0)  # Black color for the email
    pdf.cell(0, 10, email, ln=True, align='C')
    pdf.ln(10)

    # Section: Education
    pdf.set_font(font_face, 'B', 16)
    ...
    pdf.cell(0, 10, f"{random.choice(unis)}", ln=True, align='L')
    pdf.cell(0, 10, f"{random.choice(degrees)}", ln=True, align='L')
    pdf.cell(0, 10, f"Graduation Year: {grad_year}", ln=True, align='L')
    pdf.ln(10)

    # Section: Experience
    pdf.set_font(font_face, 'B', 16)
    pdf.set_text_color(0, 51, 102)
    pdf.cell(0, 10, 'Experience', ln=True, align='L')
    pdf.ln(5)

    midyear = int(grad_year + (year - grad_year) * 0.1 * random.randrange(3, 7))
    for i in range(2):
        pdf.set_font(font_face, 'B', 12)
        pdf.set_text_color(0, 0, 0)
        pdf.cell(0, 10, f"{faker.company()} ({grad_year if i == 0 else midyear} - {midyear if i == 0 else year})", ln=True, align='L')
        pdf.set_font(font_face, '', 12)
        pdf.cell(0, 10, f"{faker.job()}", ln=True, align='L')
        for _ in range(random.randint(3, 5)):
            pdf.cell(0, 10, f"- {faker.bs()}", ln=True, align='L')
        pdf.ln(5)

    # Section: Skills
    pdf.set_font(font_face, 'B', 16)
    pdf.set_text_color(0, 51, 102)
    pdf.cell(0, 10, 'Skills', ln=True, align='L')
    pdf.ln(5)
